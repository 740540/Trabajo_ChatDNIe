...
class DNIIMApplication:
    ...
    def initialize(self) -> bool:
        ...
        self.network.contact_manager = self.contacts  # Pass contact manager for pubkey storage
        self.network.start(self._on_packet_received)
        print("   mDNS advertising started")
        print("   UDP socket listening")
        
        # Initialize GUI (with DNIe photo and info)
        print("\n6. Starting GUI...")
        my_photo = self.dnie_auth.get_photo()
        self.gui = DNIGUI(my_name, my_photo, my_dni)
    ...
    def _on_packet_received(self, data: bytes, addr: Tuple[str, int]):
        ...
        try:
            ...
            if msg_type == MessageType.DISCOVERY_REQ:
                # Process discovery request (someone asking for our public key)
                response = self.protocol.process_discovery_request(data, addr)
                if response:
                    self.network.send(response, addr[0], addr[1])
                    self.gui.log(f"Sent public key to {addr[0]}", 'success')
            
            elif msg_type == MessageType.DISCOVERY_RESP:
                # Process discovery response (received public key)
                if self.protocol.process_discovery_response(data, addr):
                    self.gui.log(f"Received public key from {addr[0]}", 'success')
                    # Auto-initiate handshake now that we have the key
                    # Find the fingerprint for this address
                    for fp, peer_addr in self.network.list_peers():
                        if peer_addr == addr[0]:
                            self.gui.log(f"Auto-initiating handshake with {fp[:8]}", 'info')
                            try:
                                cid, packet = self.protocol.create_handshake_init(fp)
                                if cid > 0:  # Not a discovery message
                                    self.network.send(packet, addr[0], addr[1])
                            except Exception as e:
                                self.gui.log(f"Auto-handshake error: {e}", 'error')
                            break
            
            elif msg_type == MessageType.HANDSHAKE_INIT:
                    ...
                    try:
                        ...
                        if peer_addr:
                            ...
                            if cid == 0:
                                ...
                                chat.add_system_message("Exchanging public keys...")
                                self.gui.log(f"Sent discovery request to {contact_name}", 'info')
                            else:
                                # Normal handshake

...
class CryptoEngine:
    ...
    def __init__(self, keypair_file: str, dnie_auth=None):
        self.keypair_file = keypair_file
        self.dnie_auth = dnie_auth
        self.static_private = None
        self.static_public = None
        self.load_or_generate_keypair()
    
    def load_or_generate_keypair(self):
        """Load existing keypair or generate new one (encrypted with DNIe)"""
        if os.path.exists(self.keypair_file):
            # Load and decrypt existing keypair
            with open(self.keypair_file, 'rb') as f:
                encrypted_data = f.read()
            
            # Decrypt with DNIe
            if self.dnie_auth:
                private_bytes = self.dnie_auth.decrypt_data(encrypted_data)
                if not private_bytes:
                    print("⚠ Failed to decrypt keypair, generating new one")
                    self._generate_new_keypair()
                    return
                
                # Verify it's 32 bytes (X25519 key size)
                if len(private_bytes) != 32:
                    print("⚠ Invalid keypair size, generating new one")
                    self._generate_new_keypair()
                    return
                
                self.static_private = X25519PrivateKey.from_private_bytes(private_bytes)
                self.static_public = self.static_private.public_key()
                print("✓ Keypair loaded and decrypted with DNIe")
            else:
                # Fallback for mock mode (no encryption)
                if len(encrypted_data) == 32:
                    self.static_private = X25519PrivateKey.from_private_bytes(encrypted_data)
                    self.static_public = self.static_private.public_key()
                else:
                    # Try to decrypt with mock
                    print("⚠ Attempting mock decryption")
                    self._generate_new_keypair()
        else:
    ...
    def _generate_new_keypair(self):
        """Generate and save new keypair (encrypted with DNIe)"""
        ...
        # Encrypt with DNIe before saving
        if self.dnie_auth:
            encrypted_data = self.dnie_auth.encrypt_data(private_bytes)
            print("✓ Keypair encrypted with DNIe certificate")
        else:
            # Fallback for mock mode (no encryption)
            encrypted_data = private_bytes
            print("⚠ Keypair saved without encryption (mock mode)")
        
        # Save encrypted keypair
        with open(self.keypair_file, 'wb') as f:
            f.write(encrypted_data)
        
        print(f"✓ New keypair generated and saved to {self.keypair_file}")
